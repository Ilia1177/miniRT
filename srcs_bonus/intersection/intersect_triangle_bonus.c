...
int	intersect_triangle(t_ray *ray, t_object *tr, float *t)
{
	...
	const float		det = dot_vec3(edge1, cross_vec4(ray->d, edge2));
	t_uv			uv;
	t_vec4			s;
	...
	uv.u = (1.0f / det) * dot_vec3(s, cross_vec4(ray->d, edge2));
	if (uv.u < 0.0f || uv.u > 1.0f)
		return (0);
	uv.v = (1.0f / det) * dot_vec3(ray->d, cross_vec4(s, edge1));
	if (uv.v < 0.0f || (uv.u + uv.v) > 1.0f)
		return (0);
	if (((1.0f / det) * dot_vec3(edge2, cross_vec4(s, edge1))) < 0.001f)
		return (0);
	*t = (1.0f / det) * dot_vec3(edge2, cross_vec4(s, edge1));
	return (1);
}

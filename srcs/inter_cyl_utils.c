...
int	check_height_cylinder(t_ray *ray, t_object *cy, float *t, t_quad quad)
{
	...
	const float		h0 = dot_vec3(sub_vec4(hit0, cy->pos), cy->axis);
	const float		h1 = dot_vec3(sub_vec4(hit1, cy->pos), cy->axis);
...
int	intersect_cylinder_lateral(t_ray *ray, t_object *cy, float *t)
{
	...
	d_perp = sub_vec4(ray->d, mult_vec4(cy->axis, dot_vec3(ray->d, cy->axis)));
	oc_perp = sub_vec4(oc, mult_vec4(cy->axis, dot_vec3(oc, cy->axis)));
...
int	intersect_disk(t_ray *ray, t_vec4 center, t_object *cyl, float *t)
{
	...
	if (dot_vec3(hit_vec, hit_vec) <= (cyl->radius * cyl->radius * 1.001f))
	{
		*t = temp;
		return (1);
	}
